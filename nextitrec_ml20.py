...
def get_models(model_para, args):
    ...
    if args.is_distilling:
        ...
        model_para['dilated_channels'] = args.teacher_dilated_channels
        ...
        model_para['dilated_channels'] = args.dilated_channels
    ...
    for _epoch in trange(int(args.epochs), desc="Epoch"):
            ...
            if args.is_distilling:
                ...
                if args.distill_type != "kd" and (args.one_step or not args.pred_distill):
                    ...
                    if args.distill_type == "emd":
                        ...
                        hid_loss = emd_loss(new_student_hidden, new_teacher_hidden, new_student_attn, new_teacher_attn, mse_criterion, args, T=2)
        ...
        if args.is_distilling:
            torch.save(state, '%s/distill_ckpt_%d_%d_%d.t7' % (args.output_dir, args.block_num*4, \
                args.dilated_channels, _epoch+1))
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--block_num', default=6, type=int, help="the num of [1,4]")
    parser.add_argument('--dilated_channels', default=128, type=int)
    parser.add_argument('--device', default='cpu', type=str)
    parser.add_argument('--batch_size', default=256, type=int)
    parser.add_argument('--lr', default=0.001, type=float)
    parser.add_argument('--T_max', default=40, type=int)
    parser.add_argument('--shrink_lr', default="true", type=str2bool)
    # parser.add_argument('--L2', default=0.001, type=float)
    parser.add_argument('--weight_decay', default="0", type=float)

    parser.add_argument("--train_from_scratch", default="true", type=str2bool)
    parser.add_argument('--pretrain_model_path', type=str, \
        default='/data1/libo/checkpoint/ml20/emd/v1/ckpt_24_128_40u2.t7')

    parser.add_argument('--is_distilling', default="true", type=str2bool, help="distill or not")
    parser.add_argument('--distill_type', type=str, default="emd", help="distill type")
    ...
    parser.add_argument('--one_step', default="false", type=str2bool)
    parser.add_argument('--use_attn', default="True", type=str2bool)
    parser.add_argument("--teacher_block_num", default=12, type=int)
    parser.add_argument("--teacher_dilated_channels", default=256, type=int)
    parser.add_argument("--T", default=1, type=int, help="soft label Temperature")
    parser.add_argument("--lambda_hid", default=1, type=float, help="weight for loss between hidden states")
    parser.add_argument("--lambda_embed", default=20, type=float, help="weight for loss between hidden states")
    parser.add_argument("--alpha", default=0.7, type=float)
    parser.add_argument('--teacher_model_path', type=str, \
        default='Data/checkpoint/ml20/best_scratch_model_48_256.t7')

    parser.add_argument('--log_path', type=str, default='ml20_emd_v1u2.log')
    ...
    parser.add_argument('--data_file', type=str, default='Data/Session/mllatest_update_ls20gr3.csv',
                        help='data path')
    parser.add_argument('--epochs', default=50, type=int)
    parser.add_argument('--output_dir', default='/data1/libo/checkpoint/ml20', type=str)
    parser.add_argument('--eval_percentage', type=float, default=0.1,
                        help='0.1 means 80% training 20% testing')
    ...
    if args.is_distilling:
        assert args.distill_type in ["tinybert", "emd", "kd"] 
    if args.distill_type == "emd":
        assert args.emd_type in ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10"]
    if args.emd_type == "v5":
        args.use_attn = False
    if args.emd_type == "v7":
    ...
    model_para = {
        ...
        'rezero': False,
        'fit_size': args.teacher_dilated_channels if args.is_distilling else -1,
    ...
    logger.info("dilated_channels: {}".format(args.dilated_channels))
    if args.is_distilling:
        logger.info("teacher's dilations: {}".format(args.teacher_block_num*model_para['base_block']))
        logger.info("teacher's dilated_channels: {}".format(args.teacher_dilated_channels))
