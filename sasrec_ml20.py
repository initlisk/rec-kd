...
def do_train(model, teacher_model, train_dataloader, eval_dataloader, args, batch_num, mse_criterion, ce_criterion, kd_criterion):
    total_train_time = 0.
    global v8_weight
    lambda_embed_big = args.lambda_embed
    lambda_hid_big = args.lambda_hid
        ...
        for batch_idx, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
            ...
            if args.is_distilling:
                ...
                if args.distill_type != "kd" and (args.one_step or not args.pred_distill):
                    if args.lambda_decay == True:
                        args.lambda_embed = 0.5 * lambda_embed_big * (1 + np.cos(np.pi * batch_idx / args.T_max))
                        args.lambda_hid = 0.5 * lambda_hid_big * (1 + np.cos(np.pi * batch_idx / args.T_max))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain_model_path', type=str, \
        default='Data/checkpoint/sas/ml20/emd/v1/ckpt_24_128_50_u60.t7')
    ...
    parser.add_argument("--lambda_hid", default=0.01, type=float, help="weight for loss between hidden states")
    parser.add_argument("--lambda_embed", default=0.01, type=float, help="weight for loss between hidden states")
    parser.add_argument('--lambda_decay', default="true", type=str2bool)
    parser.add_argument("--alpha", default=0.7, type=float)
    parser.add_argument('--teacher_model_path', type=str, \
        default='Data/checkpoint/sas/ml20/best_scratch_model_48_256.t7')

    parser.add_argument('--log_path', type=str, default='sas_ml20_emd_v1_onestep_decay.log')
    ...
    parser.add_argument('--output_dir', default='Data/checkpoint/sas/ml20/tmp', type=str)
    ...
    if args.distill_type == "emd":
        assert args.emd_type in ["v1", "v2", "v3", "v4", "v5", "v6", "v7", "v8", "v9", "v10", "v11", "v12"]
    if args.emd_type == "v5":
        args.use_attn = False
    if args.emd_type == "v7":
